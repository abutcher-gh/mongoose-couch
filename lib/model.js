...
Model.find = function find(options, manipulator, callback) {
  ...
  var response_to_input;
  if (callback !== undefined)
  {
    if (options === undefined)
       options = {};
    options.include_docs = true;
    response_to_input = response_to_input_full;
  }
  else
    response_to_input = response_to_input_refs;
  self.collection.all(options, function(err, objs){
     if (err)
        return search.reject(err);
     response_to_input(objs);
     return search.fulfill(objs);
  });
  if(callback !== undefined)
     return search;
  var onReject = function(err) {
     console.log("Find operation failed but error was not caught.");
  };
