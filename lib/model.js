...
Model.discriminator = function discriminator (name, schema) {
  ...
  (function mergeSchemas(schema, baseSchema) {
    ...
    (function validateOptions(a, b) {
      ...
      ['toJSON', 'toObject', 'views', 'updates', 'forcePlainDiscriminatorIndex'].forEach(function(opt) {
        if (b[opt])
          a[opt] = b[opt];
        else
          delete a[opt];
      ...
      if (!utils.deepEqual(a, b)) {
        throw new Error("Only toJSON, toObject, views, updates and forcePlainDiscriminatorIndex are customizable on discriminator schemas");
      }
...
Model.ensureIndexes = function ensureIndexes(cb) {
  ...
  if (!indexes.length && !customViews && !customUpdates) {
