...
var Document = require('./document')
    ...
    , utils = require('./utils')
    , DiscriminatorMismatch = require('./errors/discriminator-mismatch')
...
Model.findWithView = function findWithView(view, options, callback) {
  ...
  this.collection.findWithView(view, options, function (err, docs) {
    ...
    self.hydrate(docs, view.startsWith('$root'), callback);
  });
};
...
Model.findOneWithView = function findOneWithView(view, options, callback) {
  ...
  this.collection.findOneWithView(view, options, function (err, doc) {
    ...
    if (doc)
       self.hydrate(doc, view.startsWith('$root'), callback);
    else
       callback(null, doc);
  });
};
...
Model.findById = function findById(ids, options, callback) {
  ...
  this.collection.findById(ids, options, function (err, docs) {
    ...
    self.hydrate(docs, /*queryRoot=*/false, callback);
  });
};

// Doc is assumed to come from the underlying database, but this validates
// whether it has the appropriate discriminator for the collection type; i.e.
// whether it should be included in results from a derived schema.
function isValidDocForDiscriminator(doc) {
  if (!this.schema.discriminatorMapping || this.schema.discriminatorMapping.isRoot)
    return true;
  if (doc[this.schema.discriminatorMapping.key] === this.schema.discriminatorMapping.value)
    return true;
  return false;
}
...
Model.hydrate = function (docs, queryRoot, callback) {
  var self = this;
  var docArray = Array.isArray(docs);
  docs = (docArray) ? docs : [docs];
  if (!queryRoot && self.schema.discriminatorMapping?.isRoot === false) {
    var filtered = docs.filter(isValidDocForDiscriminator.bind(self));
    if (filtered.length === 0 && docs.length > 0)
      return callback(new DiscriminatorMismatch(self.schema.discriminatorMapping.value, docs.map(d => d[self.schema.discriminatorMapping.key]).join(', ')), null);
    docs = filtered;
  }
  var arr = [];
