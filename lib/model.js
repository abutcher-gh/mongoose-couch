...
Model.discriminator = function discriminator (name, schema) {
  ...
  (function mergeSchemas(schema, baseSchema) {
    ...
    ['collection', 'autoIndex'].forEach(function(opt) {
    ...
    (function validateOptions(a, b) {
      ...
      ['toJSON', 'toObject', 'views', 'forcePlainDiscriminatorIndex'].forEach(function(opt) {
        if (b[opt])
          a[opt] = b[opt];
        else
          delete a[opt];
      ...
      if (!utils.deepEqual(a, b)) {
        throw new Error("Only toJSON, toObject, views and forcePlainDiscriminatorIndex are customizable on discriminator schemas");
      }
  ...
  self.collection.findById('_design/' + designDoc, function (err, design) {
    ...
    function maybeUpdateIndex(name, spec) {
      ...
      if (spec)
        ...
        viewName = 'by' + name.charAt(0).toUpperCase() + name.slice(1);
      if (views[viewName]
          && (spec == undefined ||
              (!spec.ver && !views[viewName].ver)
              || views[viewName].ver >= spec.ver))
        return;
      var def = {};
      if (spec) {
        if (spec.map)
        ...
        def.map = 'function(doc){'+discriminatorEarlyOut+'emit(doc.'+name+',doc);}';
      }
      views[viewName] = def;
      modified = true;
    }
    ...
    if (modified)
      self.collection.collection.save('_design/' + designDoc, design, done);
    else
